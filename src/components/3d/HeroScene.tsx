...
function CoffeeCup({ scrollY }: { scrollY: number }) {
  ...
  const goldMat = useMemo(() => new THREE.MeshStandardMaterial({
    color: "#C9A84C", metalness: 0.95, roughness: 0.1, envMapIntensity: 2,
  }), []);
  const porcelainMat = useMemo(() => new THREE.MeshStandardMaterial({
    color: "#F8F4ED", metalness: 0.0, roughness: 0.15, envMapIntensity: 1.5,
  }), []);
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      ...
      <mesh material={goldMat} rotation-x={-Math.PI / 2} position={[0, -0.47, 0]}>
